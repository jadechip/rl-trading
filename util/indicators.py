...
def add_indicators(df):
    ...
    df['KCLI'] = ta.volatility.keltner_channel_lband_indicator(df["High"],
                                                    df["Low"],
                                                    df["Close"])
    df['DCHI'] = ta.volatility.keltner_channel_hband_indicator(df["High"],
                                                    df["Low"],
                                                    df["Close"])
    df['DCLI'] = ta.volatility.keltner_channel_lband_indicator(df["High"],
                                                    df["Low"],
                                                    df["Close"])                                                    
    # df['DCHI'] = ta.volatility.DonchianChannel.donchian_channel_hband_indicator(df["Close"])
    # df['DCLI'] = ta.volatility.DonchianChannel.donchian_channel_lband_indicator(df["Close"])
